...
vec3 rgb(vec3 c){vec4 K=vec4(0.0,-1.0/3.0,2.0/3.0,-1.0),p=mix(vec4(c.bg,K.ab),vec4(c.gb,K.rg),step(c.b,c.g)),q=mix(vec4(p.rga,c.r),vec4(c.r,p.gbr),step(p.r,c.r));float d=q.r-min(q.a,q.g),e=1e-10;return vec3(abs(q.b+(q.a-q.g)/(6.0*d+e)),d/(q.r+e),q.r);}
vec3 rgb(f r, f g, f b) {return rgb(v3(r, g, b));}
vec3 rgb(f h) {return rgb(v3(h));}

vec3 hsv(vec3 c){vec4 K=vec4(1.0,0.66666,0.33333,3.0);vec3 p=abs(fract(c.rrr+K.rgb)*6.0-K.aaa);return c.b*mix(K.rrr,p-K.rrr,c.g);}
vec3 hsv(f h, f s, f v){return hsv(v3(h, s, v));}
vec3 hsv(f v){return hsv(v, 1.0, 1.0);}
"""
